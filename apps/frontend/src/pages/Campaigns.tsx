...
import { Megaphone, Plus, Calendar, CheckCircle2, Clock, Trash2, RotateCw, BarChart3, Eye, Repeat } from 'lucide-react';
...
export const Campaigns: React.FC = () => {
  ...
  const [relaunchFromCampaign, setRelaunchFromCampaign] = useState<any>(null);
  ...
  return (
    <div className="p-4 sm:p-8 space-y-6">
                    ...
                    <td className="py-4 px-6 text-right whitespace-nowrap">
                      <div className="inline-flex items-center justify-end space-x-2">
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            setRelaunchFromCampaign(camp);
                          }}
                          title="Relaunch — same template & settings, same recipients by default (or pick a different audience)"
                          className="px-2.5 py-1.5 bg-sky-500/10 hover:bg-sky-500/20 text-sky-400 border border-sky-500/20 rounded-lg text-xs font-semibold flex items-center transition-all"
                        >
      ...
      <CreateCampaignModal
        isOpen={Boolean(relaunchFromCampaign)}
        onClose={() => setRelaunchFromCampaign(null)}
        copyFrom={relaunchFromCampaign}
      />
