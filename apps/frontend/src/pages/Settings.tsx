    ...
    <div className="p-8 space-y-8 w-full">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-6 shadow-xl">
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={() => {
              const ruleName = prompt('Enter Rule Name (e.g. Greeting Rule):');
              if (!ruleName) return;
              const rawKeywords = prompt('Enter Trigger Keywords separated by commas (e.g. hi, hiii, HIIII, hello, hey):');
              if (!rawKeywords) return;
              const replyMsg = prompt('Enter Automated Reply Message to send:');
              if (!replyMsg) return;

              const keywords = rawKeywords.split(',').map(k => k.trim()).filter(Boolean);

              apiClient.post('/auto-responder', {
                name: ruleName,
                keywords,
                replyMessage: replyMsg,
              }).then(() => {
                alert('✅ Custom Auto-Responder Rule Added Successfully!');
          ...
          <table className="w-full text-left border-collapse text-xs">
            <thead>
              <tr className="bg-slate-900/60 border-b border-slate-800 text-slate-400 font-semibold uppercase tracking-wider">
                ...
                <th className="py-3 px-4">Automated Reply Message</th>
                <th className="py-3 px-4 text-right">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800 text-slate-200">
              <tr className="hover:bg-slate-900/40">
                <td className="py-3.5 px-4 font-bold text-white">Default Greeting</td>
                <td className="py-3.5 px-4">
                  <div className="flex flex-wrap gap-1">
                    {['hi', 'hiii', 'HIIII', 'hello', 'hey'].map(k => (
                      <span key={k} className="px-2 py-0.5 rounded-md bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 font-mono text-[11px]">
                        {k}
                      </span>
                    ))}
                  </div>
                ...
                <td className="py-3.5 px-4 text-slate-300">
                  👋 Hello! Welcome to our business. How can we assist you today?
                </td>
              ...
              <tr className="hover:bg-slate-900/40">
                <td className="py-3.5 px-4 font-bold text-white">Pricing Inquiry</td>
                <td className="py-3.5 px-4">
                  <div className="flex flex-wrap gap-1">
                    {['price', 'pricing', 'plan', 'cost'].map(k => (
                      <span key={k} className="px-2 py-0.5 rounded-md bg-purple-500/10 text-purple-300 border border-purple-500/20 font-mono text-[11px]">
                        {k}
                      </span>
                    ))}
                  </div>
                ...
                <td className="py-3.5 px-4 text-slate-300">
                  🏷️ Thank you for your interest! Our sales team will share our complete price list with you shortly.
                </td>
                <td className="py-3.5 px-4 text-right">
                  <span className="px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-400 text-[10px] font-bold border border-emerald-500/20">
                    ACTIVE
