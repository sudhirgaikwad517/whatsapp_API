...
export const EditTemplateModal: React.FC<EditTemplateModalProps> = ({ isOpen, onClose, template }) => {
  ...
  const [defaultMediaId, setDefaultMediaId] = useState(template?.defaultMediaId || '');
  ...
  React.useEffect(() => {
    if (template && isOpen) {
      ...
      setDefaultMediaId(template.defaultMediaId || '');
  ...
  const editMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await apiClient.patch(`/whatsapp/templates/${template.id}`, {
        name: cleanName,
        category,
        language,
        headerType,
    ...
    onError: (err: any) => {
      setError(err.response?.data?.error?.message || err.message || 'Failed to update template.');
    },
  });
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          >
                ...
                <div className="bg-slate-950 border border-slate-800 rounded-xl p-3 mt-2 space-y-2">
                  ...
                  {headerSampleFileName ? (
                    ...
                    <p className="text-[10px] text-amber-300 leading-relaxed">
                      Meta requires an actual example image to review an image-header template — needed every time you resubmit
                      changes, separate from the default campaign image below.
                    </p>
                  )}
                </div>
              )}
              ...
              {headerType === 'IMAGE' && (
                <div className="bg-slate-950 border border-slate-800 rounded-xl p-4 mt-2">
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-xs font-semibold uppercase text-slate-400">Default Campaign Image (Optional)</label>
                    <label className="text-[11px] font-bold text-emerald-400 hover:text-emerald-300 cursor-pointer">
                      <span>⚡ Upload & Attach Default Image</span>
                  ...
                  <input
                    type="text"
                    readOnly
                    placeholder="No default image attached yet"
                    value={defaultMediaId}
                    className="w-full bg-slate-900 border border-slate-800 rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-emerald-500 mb-2"
                  />
                  {isUploadingMedia && <p className="text-[10px] text-purple-400 animate-pulse">Uploading securely to Meta...</p>}
                  
                  <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-3 flex items-start">
                    <span className="text-amber-400 mr-2 text-lg leading-none">⚠️</span>
                    <p className="text-[10px] text-amber-300 leading-relaxed">
                      <strong>Meta Policy:</strong> Uploaded Media IDs expire after <strong>30 days</strong>. You will need to re-attach the image after 30 days if you plan to continue using it in campaigns. Attaching an image here will auto-fill your Campaigns, and it <strong>does not</strong> trigger a Meta review!
                    </p>
                  </div>
                </div>
              )}
            ...
            <div className="pt-3">
              <button
                type="submit"
                disabled={editMutation.isPending || (headerType === 'IMAGE' && !headerSampleHandle)}
              ...
              >
                ...
                <span>{editMutation.isPending ? 'Submitting to Meta API...' : 'Submit Template to Meta'}</span>
              </button>
            </div>
          </form>
